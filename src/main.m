...
subplot(1,3,1)
imshow(cast_int(a));
title(['Reconstrction A; PSNR = ',num2str(error_a)]);
subplot(1,3,2)
imshow(cast_int(B));
title(['Reconstrction B; PSNR = ',num2str(error_B)]);

subplot(1,3,3)
imshow(cast_int(C));
title(['Reconstrction C; PSNR = ',num2str(error_C)]);
...
f_en = figure
bar([e_0 e_1 e_2 e_3 e_4 e_5 e_6 e_7 e_8 e_9 e_10])
title('Entropy');
...
s_2 = 1.0;
t_2 = 0.2;
mask_2 = find_zero_crossing(recon_llll, s_1, t_1);
